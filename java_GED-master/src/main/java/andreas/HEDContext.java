package andreas;
// current version of HEDContext
// this class computes an (n+1) x (m+1) context cost matrix
// including deletion and insertion cost for the whole context
...
public class HEDContext {
	...
	public HEDContext(Graph g1, Graph g2, CostFunctionManager cf, int radius) {
		this.g1 = g1;
		this.g2 = g2;
		this.cf = cf;
		this.radius = radius;
	...
	public double[][] generateContextCostMatrix() {
		double[][] localCosts = getLocalCostMatrix();
	...
	private double[][] getContextCostMatrix(double[][] localCosts) {
		double[][] contextCosts = new double[g1.size()+1][g2.size()+1];
		TreeContext[] context1 = getContext(g1);
		TreeContext[] context2 = getContext(g2);
		for (int i=0; i < g1.size(); i++) {
			TreeContext contextU = context1[i];
			TreeContext contextV = new TreeContext();
			contextCosts[i][g2.size()] = getTreeEditDistance(contextU, contextV, localCosts);
		}
		for (int j=0; j < g2.size(); j++) {
			TreeContext contextU = new TreeContext();
			TreeContext contextV = context2[j];
			contextCosts[g1.size()][j] = getTreeEditDistance(contextU, contextV, localCosts);
		}
		for (int i=0; i < g1.size(); i++) {
			TreeContext contextU = context1[i];
			for (int j=0; j < g2.size(); j++) {
